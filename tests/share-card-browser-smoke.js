...
(async () => {
  ...
  try {
    ...
    const measurements = await page.evaluate(() => {
      ...
      const results = [];
      for (const language of ['en', 'zh']) for (const appearance of ['paper', 'ink', 'blueprint', 'minimal']) for (const count of [0, 1, 4, 8, 30, 100]) {
        const model = {
    ...
    for (const appearance of ['ink', 'blueprint', 'minimal']) {
      ...
      await saveImage(`share-${appearance}-v8.png`);
    }
    ...
    console.log('Share card browser smoke passed: 48 layout cases, four styles, field privacy, real PNG download, zoom, mobile and blob cleanup.');
