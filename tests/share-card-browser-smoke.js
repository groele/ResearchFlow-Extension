...
(async () => {
  ...
  try {
    ...
    const measurements = await page.evaluate(() => {
      ...
      const results = [];
      for (const language of ['en', 'zh']) for (const appearance of ['paper', 'ink', 'blueprint', 'minimal', 'cyber', 'aurora', 'terminal', 'cobalt', 'violet', 'sage', 'sand']) for (const count of [0, 1, 4, 8, 30, 100]) {
        const model = {
    ...
    for (const appearance of ['ink', 'blueprint', 'minimal', 'cyber', 'aurora', 'terminal', 'cobalt', 'violet', 'sage', 'sand']) {
    ...
    console.log('Share card browser smoke passed: 132 layout cases, eleven card styles, field privacy, real PNG download, zoom, mobile and blob cleanup.');
