...
ResearchFlow 是一个面向 Chrome 的研究资料库扩展，聚焦于一条清晰的研究链路：采集论文或观察记录，关联到项目，并在需要时检索其证据与笔记。
...
打开旧数据库不会删除其历史字段。活跃运行时仍会规范化并使用项目、研究记录、任务和证据；若要进一步清理已退役字段，请先导出 JSON 备份。
...
4. 固定 ResearchFlow 图标，在学术论文页面打开侧边栏。
